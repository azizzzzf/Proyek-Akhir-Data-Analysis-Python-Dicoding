...
all_df = pd.read_csv("C:/Users/aziz/Documents/GitHub/Proyek-Akhir-Data-Analysis-Python-Dicoding/data/all_data.csv")
all_df.sort_values(by="order_approved_at", inplace=True)
all_df.reset_index(inplace=True)

# Geolocation Dataset
geolocation = pd.read_csv('C:/Users/aziz/Documents/GitHub/Proyek-Akhir-Data-Analysis-Python-Dicoding/data/geolocation.csv')
...
with col2:
    ...
    st.markdown(f"Total Pemasukan: **{total_revenue}**")
...
plt.title("Rating Dari Pelanngan Untuk Pelayanan", fontsize=15)
plt.xlabel("Rating")
plt.ylabel("Jumlah")
...
with tab1:
    ...
    plt.title("Jumlah Pelanggan Dari Daerah", fontsize=15)
    plt.xlabel("Daerah")
...
with tab2:
    ...
    st.markdown(f"Status Pemesanan Paling Umum: **{common_status_}**")
    ...
    plt.title("Status Pemesanan", fontsize=15)
    plt.xlabel("Status")
    plt.ylabel("Jumlah")
    plt.xticks(fontsize=12)
    st.pyplot(fig)
...
with tab3:
    ...
    with st.expander("Lihat Penjelasan"):
        st.write('Berdasarkan grafik di atas, ada lebih banyak pelanggan di bagian tenggara dan selatan.')

st.caption('Copyright (C) Azis Fathur Rahman 2024')
